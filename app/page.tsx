...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-zinc-950 text-white flex flex-col justify-between relative overflow-hidden font-sansSelection selection:bg-blue-500/30 selection:text-white">
      ...
      {/* Header bar */}
      <header className="max-w-7xl mx-auto w-full px-6 py-6 flex items-center justify-between border-b border-zinc-900 z-10">
        ...
        <div className="flex items-center gap-4">
          <span className="text-xs font-bold text-zinc-500 hidden sm:inline-block">v2.1.0 (Stable)</span>
          <Link
            href="/dashboard"
            className="text-xs font-extrabold px-4 py-2 border border-zinc-800 rounded-lg bg-zinc-900/40 hover:bg-zinc-900 hover:border-zinc-700 hover:text-white text-zinc-300 transition-all cursor-pointer"
          >
            Sign In
          </Link>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-16 sm:py-24 text-center flex-1 flex flex-col justify-center items-center z-10">
        ...
        {/* Sparkle badge */}
        <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/10 text-xs font-extrabold text-blue-400 tracking-wider uppercase mb-8 shadow-[0_0_15px_rgba(59,130,246,0.1)] animate-pulse">
          <Sparkles className="h-3.5 w-3.5" /> Next-Gen Job Application Pilot
        </div>

        {/* Hero title */}
        <h1 className="text-4xl sm:text-6xl font-black tracking-tight leading-[1.1] max-w-3xl mb-6 bg-gradient-to-b from-white via-zinc-100 to-zinc-500 bg-clip-text text-transparent">
          Organize Your Professional Career Search.
        </h1>

        {/* Hero description */}
        <p className="text-base sm:text-lg font-medium text-zinc-400 max-w-2xl leading-relaxed mb-10">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <Link
            href="/dashboard"
            className="group relative flex items-center gap-2 text-sm font-extrabold bg-white text-zinc-950 hover:bg-blue-600 hover:text-white px-7 py-4 rounded-xl shadow-lg shadow-white/5 hover:shadow-blue-500/20 transition-all duration-300 cursor-pointer"
          >
            Enter Dashboard
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-sm font-extrabold text-zinc-400 hover:text-white px-6 py-4 rounded-xl transition-colors cursor-pointer"
          >
            View Documentation
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mt-20 w-full text-left">
          {[
            ...
            <div
              key={i}
              className="p-6 rounded-2xl border border-zinc-900 bg-zinc-900/20 backdrop-blur-md hover:border-zinc-800 transition-colors"
            >
              <div className="p-2.5 bg-zinc-900 rounded-xl w-fit mb-4 border border-zinc-800">
                {feat.icon}
              </div>
              <h3 className="text-sm font-extrabold mb-1.5 text-zinc-200">{feat.title}</h3>
      ...
      {/* Footer bar */}
      <footer className="max-w-7xl mx-auto w-full px-6 py-6 flex items-center justify-between border-t border-zinc-900 text-xs font-bold text-zinc-600 z-10">
